...
const badges = ['Agriculture', 'Art', 'Astronomy', 'Athletics', 'Aviator', 'Beloved', 'Boss', 'Buddy', 'Chivalrous', 'Clumsy', 'Commoner', 'Dancer', 'Detective', 'Eloquent', 'Flirty', 'Floriculture', 'Foodie', 'Geology', 'Healer', 'Hospitality', 'Inventor', 'Lazybones', 'Loner', 'Macabre', 'Merchant', 'Meteorology', 'Music', 'Nobility', 'Nurturing', 'Party', 'Performer', 'Ranger', 'Safari', 'Scholar', 'Seafarer', 'Smith', 'Tailor', 'Tamer', 'Thief', 'Traveler', 'Trickster', 'Urchin', 'Woodworker', 'Captain', 'City Guard', 'Hired Sword', 'Instructor', 'Royal Messanger', 'Scout', 'Squire', 'Templar', 'Village Guard', 'Air', 'Candle', 'Earth', 'Fire', 'Ice', 'Nature', 'Potion', 'Seeker', 'Water'] as const;
...
const getBadgeValue = (badge: typeof badges[number], wanderberry?: boolean) => {
...
const calculateRewards = (values: Inputs) => {
    const {baseReward, items, bonuses, repeatableBonuses, epicQuestBonuses, characters} = values;
            ...
            <Form.Group className="mb-3">
                ...
                <Row>
                    <Col>
                        ...
                        <Form.Control type="number" {...register("repeatableBonuses.extraCharacter")}/>
                    </Col>
                    <Col>
                        <Form.Label>Pippets</Form.Label>
                        <Form.Control type="number" {...register("repeatableBonuses.pippets")}/>
                    </Col>
                    <Col>
                        <Form.Label>Fauna</Form.Label>
                        <Form.Control type="number"{...register("repeatableBonuses.fauna")}/>
                    </Col>
                    <Col>
                        <Form.Label>Megafauna</Form.Label>
                        <Form.Control type="number" {...register("repeatableBonuses.megafauna")} />
                    </Col>
                </Row>
            </Form.Group>
                ...
                <div className="mt-3 p-3 border rounded bg-light" key={field.id}>
                    ...
                    <Row className="mb-3">
                        <Col>
                            ...
                            <Form.Select
                                {...register(`characters.${index}.badges.0`)}
                            >
                        ...
                        <Col>
                            ...
                            <Form.Select
                                {...register(`characters.${index}.badges.1`)}>
                                <option value="">Select a badge</option>
                                {badges.map((badge) => (
                                    <option key={badge} value={badge}>
                                        {badge}
                        ...
                        <Col>
                            ...
                            <Form.Select
                                {...register(`characters.${index}.badges.2`)}>
                                <option value="">Select a badge</option>
                                {badges.map((badge) => (
                                    <option key={badge} value={badge}>
                                        {badge}
